...
<form action="{{ route('edituser')}}" method="post" class="mb-[5vh]">
    ...
    <div class="flex flex-row mb-[2vh]">
        ...
        <select name="lvl" class="lg:text-[0.8vw] text-[2vw] border-[0.05vw] rounded border-[#C6C6C6] pl-[0.5vw] py-[0.5vh] ml-[0.5vw] min-w-[15vw] bg-white" required>
            <option value="0" {{ $user->NIV_ID == 0 ? 'selected' : '' }}>0 - Pas de niveau</option>
            @foreach ($levels as $level)
            <option value="{{ $level->NIV_ID }}" {{ $user->NIV_ID == $level->NIV_ID ? 'selected' : '' }}>{{ $level->NIV_ID }} - {{ $level->NIV_DESCRIPTION }}</option>
            @endforeach
        </select>
    </div>
    <div class="flex flex-row mb-[5vh] items-center">
        ...
        <div class="flex items-center">
            <input type="radio" id="oui" name="init" value="1" class="mr-[1vw]" {{ $user->UTI_EST_INIT == 1 ? 'checked' : '' }} required>
            <label for="oui" class="mr-[2vw] lg:text-[1vw] text-[2.5vw]">Oui</label>
            <input type="radio" id="non" name="init" value="0" class="mr-[1vw]" {{ $user->UTI_EST_INIT == 0 ? 'checked' : '' }} required>
